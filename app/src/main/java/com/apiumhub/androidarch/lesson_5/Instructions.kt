...
class MoveForward : Instruction {
    ...
    init {
        moves[Orientation.NORTH] = { Position(it.x, it.y.plus(1), it.orientation) }
        moves[Orientation.EAST] = { Position(it.x.plus(1), it.y, it.orientation) }
        moves[Orientation.SOUTH] = { Position(it.x, it.y.minus(1), it.orientation) }
        moves[Orientation.WEST] = { Position(it.x.minus(1), it.y, it.orientation) }
    }
